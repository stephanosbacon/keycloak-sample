...
module.exports = function(keycloakServiceUrl) {
  ...
  function createClient(accessToken, realm, clientData) {
    let options = {
      ...
      headers: {
        authorization: consBearerToken(accessToken),
  ...
  function consUrl(resource) {
    let rval = keycloakServiceUrl + resource;
    return rval;
  }

  /**
   * If the token is of the form 'Bearer ...' or 'bearer ...' it leaves alone otherwise
   * it prepends 'Bearer '
   * @param {string} accessToken - a token
   */
  function consBearerToken(accessToken) {
  ...
  return {
    ...
    consBearerToken: consBearerToken
  };
